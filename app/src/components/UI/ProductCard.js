...
const ProductCard = ({item}) => {
    ...
    return (
        <Col lg='3' md='4' className='mb-6 px-3'>
            <motion.div 
                className="product-card bg-white rounded-3xl shadow-lg hover:shadow-2xl transition-all duration-300 overflow-hidden group cursor-pointer border border-gray-100"
                whileHover={{ y: -8, transition: { duration: 0.3 } }}
                whileTap={{ scale: 0.98 }}
            >
                {/* Image Container */}
                <div className="product-image-container relative h-48 bg-gradient-to-br from-blue-50 to-indigo-100 overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-blue-400/10 to-purple-400/10"></div>
                    <motion.img
                        src={item.attributes?.image ? 'http://localhost:1337' + item.attributes.image.data[0].attributes.url : logo}
                    ...
                    <motion.button
                        onClick={addToCart}
                        className="absolute top-4 right-4 w-12 h-12 bg-white/90 backdrop-blur-sm rounded-full shadow-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300"
                        whileHover={{ scale: 1.1 }}
                        whileTap={{ scale: 0.9 }}
                    >
                        <CiShoppingCart className="text-indigo-600 text-xl" />
                    </motion.button>
                ...
                <div className="p-5">
                    {/* Product Name */}
                    <h3 className="product-name text-gray-800 font-bold text-lg mb-2 line-clamp-2 group-hover:text-indigo-600 transition-colors duration-300">
                        <Link to={`/shop/${item.id}`} className="text-decoration-none">
                    ...
                    <div className="product-description mb-4">
                        <ul className="text-sm text-gray-600 space-y-1">
                            {item.attributes.description.split("-").slice(0, 3).map((desc, index) => (
